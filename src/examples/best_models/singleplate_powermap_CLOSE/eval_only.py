...
if __name__ == "__main__":
    ...
    util_example.eval_offset_example(model=model, power_data=input_data, offset=offset,
        title=f'Gauss Power {model.engine.e()} Epochs (Offset)', save_dir=eval_dir, suff=''
    )
    util_example.eval_offset_example(model=model, power_data=input_data, normal=True, offset=offset,
        title=f'Gauss Power {model.engine.e()} Epochs (Norm, Off)', save_dir=eval_dir, suff='norm'
    )


    model.load_checkpoint(epoch=13000)
