...
if __name__ == "__main__":
    ...
    model.load_checkpoint() # first 1k were paired and PDE
    ...
    model.engine.loss_wts.set_wts(
        ...
        pinn=float(model.grid_map.shape[0]/7), paired=1e-4
    )
    ...
    # model.set_lr(1e-3)
    # loss_hist = model.train_model(train_data=train_data, epochs=1000)
    # util_example.plot_example_losshist(model, loss_hist=loss_hist.copy(), save_dir=train_dir)

    ''' introduce side Boundary Conditions '''
    model.engine.core_only = False
    model.set_lr(2e-5)
    ...
    # model.set_lr(8e-4)
    # loss_hist = model.train_model(train_data=train_data, epochs=num_epochs-10000)
    # util_example.plot_example_losshist(model, loss_hist=loss_hist.copy(), save_dir=train_dir)
